...
public:
  ...
  // Helper to speed up internal reaping while the given future is
  // not ready. The duration is how long the clock should be advanced
  // (not how long this reap action may take).
  void reapUntilResponse(
      const Future<v1::agent::Response>& response,
      const Duration& duration = Seconds(15))
  {
    Clock::pause();
    Duration reapingTime;
    while (response.isPending() && reapingTime < duration) {
      reapingTime += process::MAX_REAP_INTERVAL();
...
// 4) The isolators to enable during the test. In case the isolator requires
//    specific permissions or capabilities, the second parameter can be
//    used to supply additional test filters.
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Combine(
      ...
      ::testing::Values(
        make_tuple(std::string("posix/cpu,posix/mem"), std::string()),
        // TODO(josephw): Add the following combination:
        // make_tuple(
        //     std::string("windows/cpuset,windows/mem"),
        //     std::string("WINDOWS_")),
        make_tuple(
            std::string(
                "cgroups/cpu,cgroups/mem,filesystem/linux,namespaces/pid"),
            std::string("ROOT_CGROUPS_")))));
    ...
    AgentContainerAPITest, TwoLevelNestedContainerLaunch)
{
  ...
  reapUntilResponse(waitgrandChild);
  ASSERT_TRUE(waitgrandChild.isReady());
  ...
  reapUntilResponse(waitChild);
  ASSERT_TRUE(waitChild.isReady());
