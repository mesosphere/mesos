...
// agent's resource provider manager and reports its resources,
// the agent send an `UpdateSlaveMessage` reflecting the updated
// capacity.
//
...
TEST_F(SlaveTest, ResourceProviderSubscribe)
{
  ...
  mesos::v1::resource_provider::Call call2;
  call2.set_type(mesos::v1::resource_provider::Call::UPDATE_STATE);

  call2.mutable_resource_provider_id()->CopyFrom(resourceProviderId);

  mesos::v1::resource_provider::Call::UpdateState* updateState =
    call2.mutable_update_state();

  v1::Resource* disk = updateState->add_resources();
  disk->mutable_provider_id()->CopyFrom(resourceProviderId);
  disk->set_name("disk");
  disk->set_type(mesos::v1::Value::SCALAR);
  disk->mutable_scalar()->set_value(8096);

  // In this test scenario, it doesn't matter what we set here.
  updateState->set_resource_version_uuid("test");

  AWAIT_READY(resourceProvider.send(call2));
  ...
  expectedResources += devolve(*disk);
