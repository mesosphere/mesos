    ...
    pid_t pid)
{
  ...
  //
  // For the debug containers (see below), their OOM score adjustments will be
  // set to the same value as their parent containers.
  //   1. Debug containers (2nd level nested) under a command task container.
  //   2. Debug containers (3rd level nested) under a nested task container.
    ...
    const google::protobuf::Map<string, Value::Scalar>& resourceLimits)
{
  ...
  auto setLimitInBytes = [=]() -> Try<Nothing> {
    if (isInfiniteLimit) {
      Try<Nothing> write = cgroups::write(hierarchy, cgroup, "memory.limit_in_bytes", "-1");
      if (write.isError()) {
        return Error(
