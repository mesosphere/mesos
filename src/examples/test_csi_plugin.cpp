...
public:
    ...
    : flags(_flags)
  {
    ...
    foreach (const string& path, paths.get()) {
      Try<uint64_t> capacity = numify<uint64_t>(Path(path).basename());
      if (capacity.isError()) {
        LOG(ERROR)
      ...
      csi::CreateVolumeResponse* response) override
  {
    ...
    if (isSupportedVersion(request, response)) {
      ...
      Bytes capacity = DEFAULT_VOLUME_CAPACITY;
      ...
      csi::DeleteVolumeResponse* response) override
  {
    ...
    if (isSupportedVersion(request, response)) {
      ...
      Try<uint64_t> capacity = numify<uint64_t>(path.basename());
      ...
      csi::ListVolumesResponse* response) override
  {
    ...
    if (isSupportedVersion(request, response)) {
      ...
      if (paths.isError()) {
        ...
        for (; it != paths->end(); it++) {
          Try<uint64_t> capacity = numify<uint64_t>(Path(*it).basename());
          if (capacity.isError()) {
            LOG(ERROR)
          ...
          volume->set_capacity_bytes(capacity.get());
          volume->mutable_access_mode()->set_mode(
              os::Permissions(mode.get()).owner.w
                ? csi::AccessMode::SINGLE_NODE_WRITER
                : csi::AccessMode::SINGLE_NODE_READER_ONLY);
