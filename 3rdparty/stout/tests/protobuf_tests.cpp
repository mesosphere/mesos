...
TEST(ProtobufTest, JsonInfinity)
{
  ...
  JSON::Object object;
  object.values["d1"] = JSON::Number(std::numeric_limits<double>::infinity());
  object.values["d2"] = JSON::Number(-std::numeric_limits<double>::infinity());
  object.values["f1"] = JSON::Number(std::numeric_limits<double>::infinity());
  object.values["f2"] = JSON::Number(-std::numeric_limits<double>::infinity());

  // Check Protobuf -> JSON.
  JSON::Object object1 = JSON::protobuf(message);
  EXPECT_EQ(object, object1);
  EXPECT_EQ(expected, stringify(object1));

  // Check JSON -> Protobuf.
  Try<tests::FloatMessage> parse =
    protobuf::parse<tests::FloatMessage>(object1);

  ASSERT_SOME(parse);
  EXPECT_EQ(message, parse.get());

  // Check String -> JSON.
  Try<JSON::Object> object2 = JSON::parse<JSON::Object>(expected);
  ...
  object.values["d1"] = JSON::String("Infinity");
  object.values["d2"] = JSON::String("-Infinity");
  object.values["f1"] = JSON::String("Infinity");
  object.values["f2"] = JSON::String("-Infinity");

  EXPECT_SOME_EQ(object, object2);

  // Check JSON -> Protobuf.
  parse = protobuf::parse<tests::FloatMessage>(object2.get());
  ASSERT_SOME(parse);
  EXPECT_EQ(message, parse.get());
}
